  ...
  (function play(gameover) {
    ...
    var player = {
      ...
      fireRound: function(n) {
        ...
        if (!this.out)
          this.bullets.push({
            x: this.weapon == "Cercle de feu" ? this.x : this.x +
 Math.cos(a) * 2 * this.s,
            y: this.weapon == "Cercle de feu" ? this.y : this.y + 
Math.sin(a) * 2 * this.s,
            hit: false,
            hits: 0,
            px: Math.cos(a + n),
...
rand(-s * rand(2, 10), can.height + s * rand(2, 10));
 ...
 player.softcore : player.hardcore;
              ...
              player.say("Tu as trouvé un " + player.weapon.
toLowerCase() + "!");
            }
          }
        }
      ...
      for (let x = -1; x <= 1; x += 2) {
        ...
        ctx.strokeStyle = p.weapon == "Cercle de feu" &&
 !p.out ? "red" : "black";
...
Math.sin(a), p.s / 3, 0, 2 * Math.PI);
 ...
 Math.sin(a) * p.barrelSize * p.s);
      ...
      ctx.beginPath();
      ctx.lineWidth = p.s / 10;
      ctx.strokeStyle = p.weapon == "Cercle de feu" && !p.out ? "red" : "black";
      ctx.fillStyle = "rgb(150, 100, 50)";
...
Math.sin(a + x * 35 * Math.PI / 180) * p.s / 2, p.s / 4, 0, 2 * Math.PI);
      ...
      if (gameover) {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("VOUS ETES MORT! " + (player.zombiesKilled < 1 ? "Aucun" :
 player.zombiesKilled.toLocaleString()) + " Zombie" +
 (player.zombiesKilled == 1 ? "" : "s") + 
" Tués", can.width / 2, can.height / 2);
        ctx.font = "50px creepster";
        ctx.fillText("PRESS ENTER POUR RELANCER", can.width / 
2, 0.75 * can.height);
      } else {
